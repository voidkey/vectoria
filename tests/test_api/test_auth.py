...
@pytest.mark.asyncio
async def test_require_api_key_accepts_valid_key(monkeypatch, settings):
    _set_api_key(monkeypatch, settings, "secret")
    monkeypatch.setattr(settings, "allow_unauthenticated", False)
    assert await require_api_key(_make_request({"X-API-Key": "secret"})) is None


@pytest.mark.asyncio
async def test_require_api_key_rejects_wrong_key(monkeypatch, settings):
    _set_api_key(monkeypatch, settings, "secret")
    monkeypatch.setattr(settings, "allow_unauthenticated", False)
    with pytest.raises(AppError) as ei:
        await require_api_key(_make_request({"X-API-Key": "wrong"}))
    assert ei.value.status_code == 403
    assert ei.value.code == ErrorCode.FORBIDDEN


@pytest.mark.asyncio
async def test_require_api_key_rejects_jwt_caller(monkeypatch, settings):
    """A Bearer/JWT credential must NOT grant access to API-key-only routes."""
    _set_api_key(monkeypatch, settings, "secret")
    monkeypatch.setattr(settings, "allow_unauthenticated", False)
    with pytest.raises(AppError) as ei:
        await require_api_key(_make_request({"Authorization": "Bearer faketoken"}))
    assert ei.value.status_code == 403
    assert ei.value.code == ErrorCode.FORBIDDEN


@pytest.mark.asyncio
async def test_require_api_key_allows_dev_mode(monkeypatch, settings):
    # allow_unauthenticated=True is already set by the fixture; dev mode returns
    # before the key is ever read, so no key setup is needed.
    assert await require_api_key(_make_request({})) is None


@pytest.mark.asyncio
async def test_require_api_key_closed_when_unconfigured(monkeypatch, settings):
...
@pytest.mark.asyncio
async def test_require_api_key_rejects_empty_header(monkeypatch, settings):
    """Empty X-API-Key header is rejected (documents the `presented and ...` short-circuit)."""
    _set_api_key(monkeypatch, settings, "secret")
    monkeypatch.setattr(settings, "allow_unauthenticated", False)
    with pytest.raises(AppError) as ei:
        await require_api_key(_make_request({"X-API-Key": ""}))
    assert ei.value.status_code == 403
